...
Core 自动在经济与防御之间生产单位；每种战斗单位保留三分之一守卫 Core，其余
Vanguard/Ranger 组成连续视野覆盖的扇形远征队，搜索敌人和敌方 Core。没有可见资源
时，Worker 会按不同距离和方位分散探路，不依赖过期的雾区记忆。

## 参考策略

资源调度参考了 `VelvetEvening/ArenaHero-nearly-perfect-guide`，并按当前规则重新实现：

- 记住视野外的历史资源，重新进入真实视野确认为空时保留 3 Tick 宽限期后再删除，
  避免先锋/游侠视野边缘抖动导致刚发现的资源被误删；
- 每个 Turn 按距离重新分配互不重复的资源目标，Worker 距离目标 ≤2 时保持目标
  （粘性分配），防止最后几步在资源间震荡；
- 没有资源任务时，按 `12 → 19 → 26 → 32 → 26 → 19` 方环错位搜索；
- 使用已确认的永久障碍和有限 A* 规划下一步；
- Core 满仓时优先腾空生成格。

原仓库的固定 `4 Worker + 2 Vanguard/1 Ranger` 小队、SDK `0.2.8` 和“19 人以下避免
维护费”逻辑没有采用。当前 `v0.14` 规则没有人口维护费，本项目继续使用 SDK `0.2.9`
的动态 `unit_cost()`，并根据现有人口动态划分守军和远征队。
...
- 收到 `CORE_DAMAGED` 或发现可在短期内攻击 Core 的敌人后进入 8 Tick 警戒，
  优先 Core 回血、补盾、补齐 Vanguard/Ranger，并由守军拦截 Core 视野内的威胁；
- Worker 遇敌优先远离威胁，Core 警戒时同时远离 Core，避免把敌人带回基地；
- Vanguard 和 Ranger 各自按威胁等级保留 1/4～1/3 守军（向下取整），确保小规模
  部队也有远征队；Core 受击时全员回防；
- 远征队以相邻视野重叠的扇形前线向外推进。伤员返回 Core 治疗后，剩余单位会重新
  排位补上空缺，避免探索带出现未覆盖区域；
- 远征队发现敌人后会召集附近队友协同攻击；集结期间自卫优先，任何已受威胁或当前能够攻击的单位都会立即反击，绝不在敌人火力下原地等待；发现敌方 Core 时全队优先
  集火 Core；
- 不设置本地人口生产上限。只要 Core 资源足以支付当前动态价格和资源预留，就会继续
  在 Worker、Vanguard、Ranger 之间分配生产；
- 前期不主动拾取 Champion Beacon。只有经济、防卫、Core 状态和资源储备都达到
  安全阈值且当前无可见敌人时，才会机会性拾取。

## 稳定性机制

- 守军目标选择排除单位自身位置，已在候选防御位的守军保持不动，避免每 Tick
  重新选位导致震荡；
- Worker 路径规划不把友军位置当障碍，仅用 reserved 机制避免同一 Tick 撞到同一格，
  防止友军移动后路径反复变化；
- `_move_toward` 两轴都有距离时优先走距离更大的轴，避免 A* 返回绕远路的第一步；
- 探索方向移除 Tick 轮换，每个 Worker 有固定的方向偏好，消除原地来回走；
...
策略默认连接生产 API，使用 SDK 自带的 WebSocket 重连和安全重试。程序没有设置人口
生产上限，会按防守缺口、经济需求和兵种平衡持续生产。Vanguard 和 Ranger 按威胁等级
保留 1/4～1/3 在 Core 附近守卫（向下取整，确保有远征队），其余单位组成扇形远征队；
远征队接敌时会召集附近队友，但集结期间自卫优先，能攻击时立即反击，绝不在敌人火力下
原地等待；发现敌方 Core 时优先集火，并在受伤后撤回治疗。共享永久世界中的敌方行为
不可预测，因此不把它描述为绝对最优。
