...
- Vanguard 和 Ranger 各自保留向上取整的三分之一守卫 Core，其余单位组成远征队；
...
生产上限，会按防守缺口、经济需求和兵种平衡持续生产。Vanguard 和 Ranger 各保留
三分之一在 Core 附近巡逻，其余单位组成扇形远征队；远征队接敌时会召集附近队友，
但集结期间自卫优先，能攻击时立即反击，绝不在敌人火力下原地等待；发现敌方 Core 时优先集火，并在受伤后撤回治疗。共享永久世界中的敌方行为不可预测，因此不把
它描述为绝对最优。
